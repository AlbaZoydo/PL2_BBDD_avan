...
def random_fecha():
    ...
    return fecha_random.date() #fecha_random.strftime("%Y-%m-%d")
...
def random_concierto():
    return random.randint(1, NUM_CONCIERTOS)
...
with open("canciones.csv", "w", newline="", encoding="utf-8") as file:
    writer = csv.writer(file)
    writer.writerow(["Nombre", "Compositor", "Fecha_grabacion", "Duracion", "Cod_discos"])
    for i in range(1, NUM_DISCOS + 1):
        NUM_CANCIONES=random.randint(8, 16)
        for j in range(NUM_CANCIONES):
            disco= random.randint(1, NUM_DISCOS)
            writer.writerow([ "nombre "+str(i), random_compositor(), random_fecha(), random_duracion(), disco])
